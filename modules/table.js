      ...
      monthsByYear[row.year].forEach(month => {
        html += `
          <tr class="month-row" id="${monthRowsId}-${month.monthInYear}" hidden>
            <td data-label="Month" class="month-cell">Month ${month.monthInYear}</td>
            <td data-label="Principal amortization (PRN)"><span style="color: #b82937;">$${formatCurrency(month.principal)}</span></td>
            <td data-label="Interest cash flows (INT)"><span style="color: #0079a6;">$${formatCurrency(month.interest)}</span></td>
            <td data-label="Total mortgage cash flows (PMT)"><span style="color: #3c6ae5;">$${formatCurrency(month.totalPayment)}</span></td>
